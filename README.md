...
#### 웹 기술
- 웹 표준기술(프론트엔드): HTML5(웹 페이지 구조) + CSS 3(디자인) + JavaScript6(인터렉티브)
- 웹 `서버` 기술(백엔드): ASP.NET Core(C#|VB), SpringBoot(Java), Flask|dJango(Python), CGI(PHP, C),  Ruby,...
- 웹 서비스: 프론트엔드 + 백엔드
- 웹 브라우저 상에서 동작: 현재는 웹 브라우저 상에서만 동작하는 경계가 사라짐
...
- 스마트홈(IoT), ERP, MES, 병원예약시스템, 호텔예약, 인터넷뱅킹, 온라인서점 ...
...
- a: 링크
- b/strong i, small, sub, sup, insu/u, del: 굵은체, 이탤릭체, 작은글씨, 아래첨자, 위첨자....
...
- 시맨틱 태그
    - header, nav, ain, section, aside, article, footer 등
...
- DOM에서 쓰이는 주요 함수들
    ...
    // 객체변수
    var obj = document.getElemtById("container");
...
- CDN 방식으로 사용
    ...
    ```
- 모든 DOM 객체에 접글할때  `$(선택자|아이디|클래스)` 로 처리
